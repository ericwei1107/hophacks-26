...
### Step L5 — Evaluator retarget
Delta-v thresholds to 12.4/12.7. Add the two new scenarios. Promote Weak Upper Stage. Extend `MissionResultClass`. This is `OUTCOME_SYSTEM_PLAN.md` Steps 12–14 executed against lunar numbers — do not fork that work, retarget it.

**Gate:** every active scenario has a fixture and deterministic positive and negative tests.

### Step L6 — Moon in the renderer
Protocol field, compression map, Unity DTO, `Moon.tsx`, cruise camera, transfer arc, the compression label.

**Gate:** `unityContract.test.ts` passes with the Moon field present; the ascent view is pixel-unchanged below 2,000 km; the compression ratio is visible on screen in the cruise view.

### Step L7 — Calibration
Sweep for the "high TWR" boundary, the periselene aim corridor, cryogenic engine thrust, and a stable fixture for every active scenario. Confirm nearby perturbations do not make the diagnosis oscillate.

**Gate:** no active scenario depends on an uncalibrated adjective.

### Step L8 — Carry-through and debrief
Worker serialization of `TransferResult`, storage schema bump, robustness counts over arrival classes, the arrival report, the parking-orbit consolation path, and debrief copy including the trip-time-versus-surplus-delta-v relationship.

**Gate:** a lunar arrival, a `tli_shortfall` with its satellite mission, and a restored saved run all agree on outcome and diagnosis.
