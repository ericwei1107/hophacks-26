...
**Both the browser and the Python backend (`api.py`) now write to and read
from the same SpacetimeDB store — one source of truth, not two databases.**
Python connects via SpacetimeDB's built-in PostgreSQL wire protocol for
reads (`db.py`, using `psycopg`) and via HTTP reducer calls for writes, by
deliberate choice — see "Python client (db.py)" below for why it isn't
direct SQL writes both ways.

The primary persistence path is now server-side: `payload_analysis.py`'s
`analyze_launched_payload()` — the function `api.py`'s `/api/analyze`
endpoint calls — persists its own full-fidelity result (real IGEL
emissions, real regulatory check, not the condensed catalog below) right
after computing it. The browser only calls `persistence/spacetime.ts`
itself as a fallback, on the path taken when the Python backend is down
and `DebriefScreen.tsx` falls back to the local TypeScript Monte Carlo —
see `web/src/api/pythonBackend.ts` for that primary/fallback split, which
predates and is unrelated to this SpacetimeDB work.
...
payload_analysis.py           analyze_launched_payload() calls
                              persist_mission_analysis() right after
                              computing its own full-fidelity result
api.py                        FastAPI /api/analyze — the HTTP surface
                              DebriefScreen.tsx calls; unrelated to
                              SpacetimeDB itself
```
...
- **Writes are HTTP reducer calls**
  ...
  `spacetime call` does), not direct `INSERT`. Every table
  `persist_mission_analysis` writes to (`mission`, `weather_snapshot`,
  `trajectory_result`) has a
...
- **Reads use `psycopg` against the Postgres wire protocol
  ...
  `/api/analyze` response it's called from.
...
`persist_mission_analysis` (bypassing `/api/analyze`'s own FastAPI/NOAA/
IGEL layers, to isolate the SpacetimeDB integration itself): all five
rows — `mission`, `weather_snapshot`, `trajectory_result`,
`emissions_estimate`, `regulatory_compliance` — were written and linked
by the same `mission_id`, with `estimate_emissions`'s analog match and
payload-share clamp and `evaluate_compliance`'s 5-year-limit verdict both
arithmetically correct against the input.
...
catalog), which may differ slightly from what `payload_analysis.py`
itself computes and returns in the `/api/analyze` response (real IGEL
emissions via `estimate_mission_footprint`, real `evaluate_mission_compliance`)
— same known gap as "What's still a placeholder" below, not something
this Python integration introduces. The HTTP response the browser sees
is always Python's own full-fidelity numbers; only the *persisted copy*
in SpacetimeDB uses the shared store's condensed catalog.
...
5. `spacetime login` once (writes `~/.config/spacetime/cli.toml`, which
   ...
   requirements.txt && python -m uvicorn api:app --reload --port 8000`.
   Every `/api/analyze` call now persists to the shared store. Override
   with `SPACETIMEDB_TOKEN`, `SPACETIMEDB_HTTP_URL`, `SPACETIMEDB_NAME`,
   `SPACETIMEDB_PG_HOST`, or `SPACETIMEDB_PG_PORT` env vars if not using
   the local defaults.
